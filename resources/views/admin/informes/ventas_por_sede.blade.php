...
<div class="report-container">
    <div class="report-header">
        <h1>Ventas por Sede</h1>
        <div class="breadcrumb">Informes / Ventas / Distribución Geográfica</div>
    </div>

    <div class="filter-bar">
        <form method="GET" action="{{ url('admin/informes/ventas-por-sede') }}" class="date-form">
            <div class="group">
                <label>Desde</label>
    ...
    <div class="grid-stats">
        <div class="stat-card">
            ...
            <div class="stat-desc">Puntos de venta con actividad</div>
        </div>
        <div class="stat-card">
            <h3>Total Consolidado</h3>
            <div class="stat-value" style="color:#1a73e8;">$ {{ number_format($data->sum('total'), 0) }}</div>
            <div class="stat-desc">Venta total todas las sedes</div>
        </div>
    </div>
    ...
    <div class="card-table" style="margin-top:30px;">
        ...
        <table class="data-table">
            ...
            <tbody>
                <?php $totalVentas = $data->sum('total'); ?>
                @foreach($data as $row)
                <?php $percentage = $totalVentas > 0 ? ($row->total / $totalVentas) * 100 : 0; ?>
                <tr>
                    <td><div style="font-weight:700; color:#111827;">{{ $row->sede ?: 'Sede Principal' }}</div></td>
                    <td style="text-align:center;">{{ $row->transacciones }}</td>
