...
<div class="report-container">
    <div class="report-header">
        <h1>Reporte de Novedades Especialistas</h1>
        <div class="breadcrumb">Informes / Especialistas / Historial Novedades</div>
    </div>

    <div class="filter-bar">
        <form method="GET" action="{{ url('admin/informes/reporte-novedades-especialistas') }}" class="date-form">
            <div class="group">
                <label>Desde</label>
    ...
    <div class="grid-stats">
        <div class="stat-card">
            ...
            <div class="stat-desc">{{ count($novedades) }} registros en el periodo</div>
        </div>
        <div class="stat-card">
            <h3>Promedio Novedad</h3>
            <div class="stat-value">$ {{ number_format(count($novedades) > 0 ? $total / count($novedades) : 0, 0) }}</div>
            <div class="stat-desc">Impacto por registro</div>
        </div>
        <div class="stat-card">
            <h3>Auditoría</h3>
            <div class="stat-value" style="color:#6366f1;">Activa</div>
            <div class="stat-desc">Control de egresos nómina</div>
        </div>
    </div>
    ...
    <div class="card-table" style="margin-top:30px;">
        <div class="card-header">
            <h3>Historial de Deducciones y Adelantos</h3>
        </div>
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td><div style="font-weight:600;">{{ date('d M, Y', strtotime($nov->date)) }}</div></td>
                    <td><div style="font-weight:700; color:#1a73e8;">{{ $nov->specialist_name }}</div></td>
                    <td><span style="background:#fef2f2; color:#991b1b; padding:4px 8px; border-radius:6px; font-size:11px; font-weight:700; text-transform:uppercase;">{{ $nov->reason ?: 'Deducción / Adelanto' }}</span></td>
                    <td style="font-size:13px; color:#6b7280; max-width:300px;">{{ $nov->notes ?: 'Sin notas adicionales' }}</td>
                    <td style="text-align:right; font-weight:900; color:#ef4444;">$ {{ number_format($nov->amount, 0) }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" style="text-align:center; padding:50px; color:#9ca3af;">No hay historial de novedades para este periodo</td>
                </tr>
                @endforelse
            </tbody>
        </table>
