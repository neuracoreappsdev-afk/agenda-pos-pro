...
<div class="report-container">
    <div class="report-header">
        ...
        <div class="breadcrumb">Informes / Ventas / Evolución Diaria</div>
    </div>

    <div class="filter-bar">
        <form method="GET" action="{{ url('admin/informes/ventas-por-dia') }}" class="date-form">
            <div class="group">
                <label>Desde</label>
            ...
            <button type="submit" class="btn-filter">Ver Evolución</button>
        </form>
    </div>
    ...
    <div class="grid-stats">
        <div class="stat-card">
            ...
            <?php $topDay = $data->sortByDesc('total')->first(); ?>
            <div class="stat-value" style="color:#10b981;">
                {{ $topDay ? date('d', strtotime($topDay->day)) . ' ' . $meses_abr[date('M', strtotime($topDay->day))] : 'N/A' }}
            </div>
            <div class="stat-desc">$ {{ $topDay ? number_format($topDay->total, 0) : 0 }} recaudados</div>
        </div>
        <div class="stat-card">
            <h3>Promedio Diario</h3>
            <div class="stat-value" style="color:#1a73e8;">$ {{ number_format($data->count() > 0 ? $data->sum('total') / $data->count() : 0, 0) }}</div>
            <div class="stat-desc">Venta media por jornada</div>
        </div>
    </div>

    <div class="card-table" style="margin-top:30px;">
        <div class="card-header">
            <h3>Histórico de Ventas Diarias</h3>
        </div>
        <table class="data-table">
            <thead>
                <tr>
                    <th>Fecha</th>
                    <th style="text-align:center">Atenciones / Ventas</th>
                ...
                <tr>
                    <td>
                        <div style="font-weight:700;">
                            {{ $dias[date('l', strtotime($row->day))] }}, {{ date('d', strtotime($row->day)) }} {{ $meses_abr[date('M', strtotime($row->day))] }} {{ date('Y', strtotime($row->day)) }}
                        </div>
                    </td>
                    ...
                    <td style="text-align:right; color:#6b7280;">$ {{ number_format($row->transacciones > 0 ? $row->total / $row->transacciones : 0, 0) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
